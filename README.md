...
> **Project status:** documentation and planning only. The application has not been bootstrapped, dependencies have not been selected, and the commands described below are not available yet.
...
The implementation will use Electron, TypeScript, Vite, electron-vite, React, Vitest, Playwright, ESLint, Prettier, electron-builder, and pnpm. Exact versions are deliberately not recorded yet. They must be resolved together at bootstrap time using the compatibility procedure in [`docs/development.md`](./docs/development.md), then pinned exactly in the manifest and lockfile.
...
These are target interfaces, not currently runnable commands.
...
No implementation work is included in the repository at this stage.
